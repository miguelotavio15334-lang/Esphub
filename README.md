-- MIIIGUEX HUB - FARM 100K+ AFK INFINITO
...
getgenv().MIIIGUEX_DATA.autoHop = true
...
local function ServerHop()
    SG:SetCore("SendNotification",{Title="MIIIGUEX HUB", Text="Sem 100K+, trocando de server!", Duration=2})
...
local noclip=true local autoHop=true local savedCFrame=nil local roubados=0 local pausado=false
    ...
    for _,plot in pairs(Plots:GetChildren()) do
        if plot:FindFirstChild("Owner") and plot.Owner.Value~=LP.Name then
            for _,m in pairs(plot:GetChildren()) do
                if m:IsA("Model") and m.PrimaryPart and m:FindFirstChild("PricePerSecond") and m:FindFirstChild("PricePerSecond").Value >= 100000 then
...
UIS.InputBegan:Connect(function(i,gp) if gp then return end if i.KeyCode==Enum.KeyCode.M then toggleGUI() elseif i.KeyCode==Enum.KeyCode.R then pausado=not pausado bStatus.Text=pausado and "PAUSADO [R]" or "STATUS: FARM ON" SG:SetCore("SendNotification",{Title="MIIIGUEX HUB", Text=pausado and "Pausado" or "Voltou a farmar!", Duration=2}) end end)
...
task.spawn(function()
    ...
    SG:SetCore("SendNotification",{Title="MIIIGUEX HUB", Text="Farm 100K+ iniciado!", Duration=4})
    while true do task.wait(0.1)
        if pausado then task.wait(0.5) continue end
        pcall(function()
            ...
            if tool then
                ...
                task.wait(0.4)
                if char:FindFirstChildWhichIsA("Tool") then char:FindFirstChildWhichIsA("Tool").Parent=LP.Backpack roubados=roubados+1 bCont.Text="ROUBADOS: "..roubados bStatus.Text="ROUBOU! Procurando próximo..." SG:SetCore("SendNotification",{Title="💰 ROUBOU!", Text="Total: "..roubados, Duration=3}) task.wait(1) end
            else
                local best,val = temPalhaco100k()
                if best then
                    bStatus.Text="ACHOU $"..val.." - "..best.Name
                    hrp.CFrame=best.PrimaryPart.CFrame+Vector3.new(0,0,2.5)
                    task.wait(0.2)
                    ...
                    bStatus.Text="Sem 100K+ trocando em 3s..."
                    task.wait(3)
